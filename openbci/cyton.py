"""
Core class for OpenBCI Cyton board.
"""
...
class OpenBCICyton(object):
    ...
    The OpenBCICyton class interfaces with the Cyton Dongle and the Cyton board to parse the data
    received and output it to Python as a OpenBCISample object.

    Args:
        port: A string representing the COM port that the Cyton Dongle is connected to.
        e.g for Windows users 'COM3', for MacOS or Linux users '/dev/ttyUSB1'.
        If no port is specified it will try to find the first port available.
        ...
        max_packets_skipped: int noting how many packets can be dropped before reconnect attempt.
    ...
    def __init__(self, port=None, daisy=False, baud=115200, timeout=None, max_packets_skipped=1):
        ...
        self.read_state = 0
        # used for daisy
        self.last_odd_sample = OpenBCISample(-1,
                                             [], [], self.start_time, self.board_type)
        ...
        for rep in range(maxbytes2skip):
            ...
            if self.read_state == 0:
                ...
                if struct.unpack('B', b)[0] == START_BYTE:
                    if rep != 0:
                        print(f'Skipped {rep} bytes before start found')
                        rep = 0
            ...
            # Read End Byte
                ...
                if val == END_BYTE:
                    sample = OpenBCISample(
                        packet_id, channels_data, aux_data, self.start_time, self.board_type)
                    self.packets_dropped = 0
                    return sample
                else:
        ...
        while self.streaming:
            ...
            # When daisy is connected wait to concatenate two samples
                ...
                # Check if the next sample ID is concecutive, if not the packet is dropped
                    ...
                    sample_with_daisy = OpenBCISample(
                        sample.id,
                        sample.channels_data + self.last_odd_sample.channels_data,
                        avg_aux_data,
                        self.start_time, self.board_type)
